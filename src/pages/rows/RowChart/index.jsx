...
export default function RowChart({
  ...
  const isFullChart =
    ...
    (window.location.pathname === "/chart" || window.location.pathname.startsWith("/chart"));

  const [state, setState] = useState({ symbol: defaultSymbol, timeframe: defaultTimeframe, range: null });

  // Defaults: EMAs + Volume ON; others OFF
  const DEFAULT_IND = {
    showEma: true, ema10: true, ema20: true, ema50: true,
    volume: true, smi: false, moneyFlow: false, luxSr: false, swingLiquidity: false,
  };
  ...
  const theme = useMemo(() => ({
    ...
    timeScale: {
      borderVisible: true, borderColor: "#2b2b2b", rightOffset: 6, barSpacing: 12,
      fixLeftEdge: true, timeVisible: true, secondsVisible: false,
    },
    crosshair: { mode: 0 },
  ...
  useEffect(() => {
    if (!onStatus) return;
    if (loading) onStatus("loading"); else if (error) onStatus("error"); else if (bars.length) onStatus("ready"); else onStatus("idle");
  ...
  useEffect(() => {
    ...
    if (ind.volume) { volRef.current = createVolumeOverlay({ chart }); volRef.current.setBars(bars); }
  ...
  useEffect(() => {
    ...
    if (ind.smi) { smiRef.current = createSmiOverlay({ chart, kLen: 12, dLen: 7, emaLen: 5 }); smiRef.current.setBars(bars); }
  ...
  useEffect(() => {
    ...
    if (ind.luxSr) {
      luxRef.current = createLuxSrOverlay({
        chart, leftBars: 15, rightBars: 15, volumeThresh: 20,
        pivotLeftRight: 5, minSeparationPct: 0.25, maxLevels: 10,
        lookbackBars: 800, markersLookback: 300,
      });
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, overflow: "hidden", background: "#0a0a0a", border: "1px solid #2b2b2b", borderRadius: 12, display: "flex", flexDirection: "column" }}>
      <Controls
        symbols={SYMBOLS}
        timeframes={TIMEFRAMES}
        value={{ symbol: state.symbol, timeframe: state.timeframe, range: state.range, disabled: loading }}
        onChange={(patch) => setState(s => ({ ...s, ...patch }))}
        onTest={showDebug ? async () => { const r = await refetch(true); alert(r.ok ? `Fetched ${r.count || 0} bars` : `Error: ${r.error || "unknown"}`); } : undefined}
      />

      <IndicatorsToolbar
        showEma={ind.showEma} ema10={ind.ema10} ema20={ind.ema20} ema50={ind.ema50}
        volume={ind.volume}
        smi={isFullChart ? ind.smi : false} showSmiToggle={isFullChart}
        moneyFlow={ind.moneyFlow} luxSr={ind.luxSr} swingLiquidity={ind.swingLiquidity}
      ...
      <div className="chart-shell" style={{ flex: "1 1 auto", minHeight: 0, display: "flex", flexDirection: "column" }}>
        <div ref={containerRef} className="tv-lightweight-charts" style={{ position: "relative", width: "100%", height: "100%", minHeight: 0, flex: 1 }} data-cluster-host>
