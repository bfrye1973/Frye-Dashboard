...
// v3.4 — white labels, flexible height, tv-lightweight-charts host
...
export default function RowChart({
  ...
  const [ind, setInd] = useState({ showEma: true, ema10: true, ema20: true, ema50: true });
  ...
  useEffect(() => {
    if (!chart) return;
    const removeAll = () => { Object.values(emaOverlaysRef.current).forEach(o => o?.remove?.()); emaOverlaysRef.current = {}; };
    removeAll();
    if (ind.showEma) {
  ...
  useEffect(() => { Object.values(emaOverlaysRef.current).forEach(o => o?.setBars?.(bars)); },
