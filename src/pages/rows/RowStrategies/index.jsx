 ...
 * - Uses normalized index IDs: I:SPX, I:NDX, I:DJI
 */
export default function RowStrategies() {
  ...
  const alignmentView = useMemo(() => {
    ...
    if (!s) {
      return { statusText: align.loading ? "Loading…" : "Flat", tone: "muted", score: 0, last: "—", failing: [], liveStatus: align.status === "live" ? "LIVE" : "MOCK" };
    }
  ...
  return (
    <div style={S.wrap}>
      ...
      <Card
        ...
        pl="—"
        actions={[{ label: "Top Candidate (Daily)", onClick: () => setSelection({ symbol: "SPY", timeframe: "1d", strategy: "wave3" }) }]}
      />
      ...
      <Card
        ...
        pl="—"
        actions={[{ label: "Top Candidate (Daily)", onClick: () => setSelection({ symbol: "SPY", timeframe: "1d", strategy: "flag" }) }]}
      />
    </div>
  );
}

function Card({ title, timeframe, rightPills = [], score = 0, last, pl, actions = [], footNote = "", bottomTabs = [] }) {
    ...
    <div style={S.card}>
      <div style={S.head}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {rightPills.map((p, i) => <span key={i} style={{ ...S.pill, ...toneStyles(p.tone).pill }}>{p.text}</span>)}
        </div>
      </div>
      ...
      <div style={S.ctaRow}>
        {actions.map((a, i) => <button key={i} onClick={a.onClick} style={S.btnSm}>{a.label}</button>)}
      </div>

      {bottomTabs.length ? (
        <div style={S.tabRow}>
          {bottomTabs.map((t) => <button key={t.k} onClick={t.on} style={S.tab}>{t.k}</button>)}
        </div>
      ) : null}
    </div>
  );
