...
// v2 — STRICT backend bind for Alignment card (no dynamic proxy)
// - Polls:  {API}/api/signals?strategy=alignment&limit=1&t=...  (no-store)
// - Maps payload to {status, signal{direction, streak_bars, confidence, timestamp, failing[]}}
// - Keeps existing layout/footprint exactly (no added height)
...
const S = {
  ...
  card: {
    ...
    gap: 8,
    minHeight: 110, // keep compact to protect Row 6 height
  },
  head: {
    display: "flex",
    alignItems: "center",
  ...
  return (
    <div style={S.card}>
      ...
      {bottomTabs.length ? (
        <div style={S.tabRow}>
          {bottomTabs.map((t) => (
            <button key={t.k} onClick={t.on} style={S.tab}>{t.k}</button>
          ))}
        </div>
      ) : null}
    </div>
...
export default function RowStrategies() {
  ...
  // Alignment state (strict)
  const [align, setAlign] = useState({ loading: true, status: "mock", signal: null, err: null });

  // Poll Alignment latest from backend directly
  useEffect(() => {
    const ctrl = new AbortController();
    async function load() {
      const url = `${API}/api/signals?strategy=alignment&limit=1&t=${Date.now()}`;
      try {
        const res = await fetch(url, { cache: "no-store", signal: ctrl.signal });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const j = await res.json();

        // Expect either {status, items:[{...}]} or {status, signal:{...}}
        const status = (j?.status || "mock").toLowerCase(); // "live" | "mock"
        const sig = Array.isArray(j?.items) ? j.items[0] : j?.signal;
        const signal = sig
          ? {
              direction: sig.direction || "none",          // "long" | "short" | "none"
              streak_bars: Number(sig.streak_bars || 0),   // debounce/streak
              confidence: Number(sig.confidence || 0),     // 0..100
              timestamp: sig.timestamp || sig.ts || null,  // ISO
              failing: Array.isArray(sig.failing) ? sig.failing : [],
            }
          : null;

        setAlign({ loading: false, status, signal, err: null });
        console.log("[RowStrategies] alignment", { url, status, hasSignal: !!signal });
      } catch (e) {
        setAlign((p) => ({ ...p, loading: false, err: String(e) }));
      }
    }
    load();
  ...
  // Derive view model (unchanged sizing)
  const alignmentView = useMemo(() => {
    const s = align.signal;
    const liveStatus = align.status === "live" ? "LIVE" : "MOCK";

    if (!s) {
      return {
        liveStatus,
        statusText: align.loading ? "Loading…" : (align.err ? "Unavailable" : "Flat"),
        tone: align.err ? "warn" : "muted",
        score: 0,
        last: "—",
        failing: [],
      };
    ...
    <div style={S.wrap}>
      ...
      <Card
        ...
        bottomTabs={[
          ...
          { k: "SPX", on: () => load("I:SPX", "10m", "alignment") },
          { k: "NDX", on: () => load("I:NDX", "10m", "alignment") },
          { k: "DJI", on: () => load("I:DJI", "10m", "alignment") },
        ]}
      />

      {/* Wave 3 (Daily) — placeholder (wire later) */}
      <Card
      ...
      {/* Flagpole (Daily) — placeholder (wire later) */}
      <Card
