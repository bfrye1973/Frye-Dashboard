...
function SectorCard({ sector, outlook, spark, last, deltaPct }) {
  ...
  if ((_last === null || _deltaPct === null) && arr.length >= 2) {
    ...
    if (_last === null) _last = lst;
    if (_deltaPct === null) _deltaPct = ((lst - first) / base) * 100;
  }
  ...
  const arrow =
    Math.abs(_deltaPct) < 0.5 ? "→" : _deltaPct > 0 ? "↑" : "↓";
  ...
  return (
    <div className="panel" style={{ padding: 10 }}>
      ...
      >
        <span>
          Last: <strong>{Number.isFinite(_last) ? _last.toFixed(1) : "—"}</strong>
        </span>
        <span className={deltaClass}>
          {arrow} {Number.isFinite(_deltaPct) ? _deltaPct.toFixed(1) : "0.0"}%
        </span>
      </div>
      <Sparkline data={arr} />
    </div>
...
function fromSectors(json) {
  ...
  const list = Object.keys(obj).map((k) => {
    ...
    return {
      ...
      outlook:
        sec?.outlook ??
        (netNH > 0 ? "Bullish" : netNH < 0 ? "Bearish" : "Neutral"),
