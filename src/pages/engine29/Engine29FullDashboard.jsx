...
function StateCard({ label, value, subtitle, accent }) {
  ...
  return (
    <Card style={{ borderColor: `${color}66`, background: `${color}0E` }}>
      <div style={{ color: "#94a3b8", fontSize: 13, fontWeight: 900, textTransform: "uppercase" }}>
        {label}
      </div>
      <div style={{ marginTop: 7, color, fontSize: 28, lineHeight: 1.05, fontWeight: 900 }}>
        {clean(value)}
      </div>
      {subtitle ? (
        <div style={{ marginTop: 7, color: "#cbd5e1", fontSize: 14, lineHeight: 1.35 }}>
          {subtitle}
        </div>
      ) : null}
    </Card>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(180px, 1fr))", gap: 10 }}>
        {items.map(([label, value]) => (
          <div key={label} style={{ border: "1px solid rgba(148,163,184,0.17)", borderRadius: 10, padding: 10, background: "rgba(2,6,23,0.28)" }}>
            <div style={{ color: "#94a3b8", fontSize: 12, fontWeight: 850, textTransform: "uppercase" }}>{label}</div>
            <div style={{ marginTop: 5, color: stateColor(value), fontSize: 17, fontWeight: 900 }}>{clean(value)}</div>
          </div>
        ))}
      </div>
    </Card>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, minmax(125px, 1fr))", gap: 8 }}>
        {[
          ...
          <div key={label} style={{ padding: 9, borderRadius: 9, background: "rgba(2,6,23,0.34)", border: "1px solid rgba(148,163,184,0.16)" }}>
            ...
            <div style={{ marginTop: 4, color: stateColor(value), fontSize: 15, fontWeight: 900 }}>{clean(value)}</div>
          </div>
        ))}
      </div>
    </Card>
    ...
    <Card style={{ borderColor: `${stateColor(moveCharacter)}66` }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.25fr 0.75fr", gap: 14 }}>
        ...
        <div style={{ display: "grid", gap: 5, alignContent: "start" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#94a3b8" }}>Direction</span><strong style={{ color: stateColor(moveDirection) }}>{clean(moveDirection)}</strong></div>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#94a3b8" }}>Pressure</span><strong style={{ color: stateColor(pressure) }}>{clean(pressure)}</strong></div>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#94a3b8" }}>ES contract</span><strong>{data?.dataQuality?.esResolvedSymbol || data?.esResolvedSymbol || es?.resolvedSymbol || "—"}</strong></div>
          {Number.isFinite(Number(es?.pointMove)) && <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}><span style={{ color: "#94a3b8" }}>30m ES move</span><strong>{Number(es.pointMove).toFixed(2)} pts</strong></div>}
        </div>
      </div>
    </Card>
  );
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!fullRes.ok || fullJson?.ok === false) throw new Error(fullJson?.error || `Engine 29 HTTP ${fullRes.status}`);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#020617", color: "#e5e7eb", padding: "18px 24px 32px", fontFamily: FONT, overflowX: "auto" }}>
      <div style={{ width: "96vw", maxWidth: 2350, margin: "0 auto", display: "grid", gap: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 18, alignItems: "center", borderBottom: "1px solid rgba(148,163,184,0.26)", paddingBottom: 10 }}>
          <div>
            <div style={{ fontSize: 30, lineHeight: 1.1, fontWeight: 900, color: "#f8fafc" }}>
        ...
        {status === "LOADING" && !data && <div style={{ color: "#94a3b8" }}>Loading Engine 29…</div>}
        {status === "ERROR" && <Card style={{ borderColor: "rgba(239,68,68,0.45)" }}><div style={{ color: "#fecaca" }}>Engine 29 dashboard error: {error}</div></Card>}
        ...
        {(data || summary) && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <EsMoveCard data={d} />
              <PressureCard display={display} move={d?.tacticalCharacter} />
            </div>
