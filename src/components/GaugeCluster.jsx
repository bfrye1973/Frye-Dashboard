...
function BigGauge({ theme = "tach", label, value = 0, withLogo = false }) {
  ...
  const MAJOR_STEP = SWEEP / (MAJOR_COUNT - 1);
  ...
  for (let k = 0; k < MAJOR_COUNT; k++) {
    ...
    if (k < MAJOR_COUNT - 1) {
      const step = MAJOR_STEP / 5;
      for (let j = 1; j <= 4; j++) {
        allTicks.push({ angle: majorAngles[k] + j * step, major: false });
      }
    }
  }
  ...
  const speedNums = Array.from({ length: 10 }, (_, i) => (i + 1) * 20);
  const numeralRadius = 77;
  const numeralAngle = (idx, total) => SWEEP_START + (idx / (total - 1)) * SWEEP;
  const toXY = (deg) => {
