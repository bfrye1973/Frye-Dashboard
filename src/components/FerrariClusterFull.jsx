...
/**
 * FerrariCluster.jsx — compact Ferrari-style cluster (clamped)
 * - id="cluster" so global CSS clamp applies; inline clamp as backup
 * - RPM (yellow), Speed (black), Water/Oil/Fuel minis, dim engine lights
 */
export default function FerrariCluster({
  ...
  oilValue = 55,
  fuelValue = 73,
  height = 360, // compact default
}) {
  const H = Math.min(height, 520); // hard cap
  return (
    <div
      id="cluster"
      style={{
    ...
    >
      ...
      <div
        style={{
          ...
          alignItems: "center",
          height: "calc(100% - 60px)", // leave room for lights
        }}
      >
        {/* RPM + minis (left) */}
        <div style={{ display: "flex", gap: 18, alignItems: "center", justifyContent: "flex-start" }}>
          ...
          >
            <MiniGaugeBlack label="WATER" value={waterValue} size={90} suffix="%" />
            <MiniGaugeBlack label="OIL" value={oilValue} size={90} suffix="%" />
            <MiniGaugeBlack label="FUEL" value={fuelValue} size={90} suffix="%" greenToRed />
          </div>
        </div>
...
function FerrariRPMGauge({ value = 5200, min = 0, max = 9000, size = 240 }) {
  ...
  for (let k = 0; k <= 9; k++) {
    ...
    majors.push(<line key={`rpm-maj-${k}`} x1={x0} y1={y0} x2={x1} y2={y1} stroke="#fff" strokeWidth="2" />);
  }
  // minors every 100 except on majors
  const minors = [];
  for (let k = 0; k <= 90; k += 10) {
    ...
    minors.push(<line key={`rpm-min-${k}`} x1={x0} y1={y0} x2={x1} y2={y1} stroke="#fff" strokeWidth="1.5" />);
  }
  // numerals
  const numerals = [];
  for (let k = 0; k <= 9; k++) {
    ...
    numerals.push(
      <text key={`rpm-num-${k}`} x={tx} y={ty + 4} textAnchor="middle" fontSize="12" fontWeight="700" fill="#0a0a0a">
        {k}
      </text>
    );
  }
  ...
  return (
    <svg
      ...
      role="img" aria-label="RPM Gauge" style={{ overflow: "hidden" }}
    >
      <defs>
      ...
      {/* red tach sweep */}
      <path d={arcPath(cx, cy, R_TICKS - 16, START, currentAngle)} stroke="#ef4444" strokeWidth="6" fill="none" strokeLinecap="round" />
...
function FerrariSpeedGauge({ value = 70, min = 0, max = 220, size = 260 }) {
  ...
  const R_FACE = 90, R_TICKS = 94, R_NUM = 68;
  const START = -120, END = 120;
  const clamp = (n, lo, hi) => Math.max(lo, Math.min(hi, n));
  const t = v => (clamp(v, min, max) - min) / (max - min);
  const currentAngle = START + (END - START) * t(value);

  const majors = [], minors = [], numerals = [];
  for (let k = 0; k <= max; k += 20) {
    const a = START + ((END - START) * (k - min)) / (max - min);
    const [x0, y0] = polarToXY(cx, cy, R_TICKS - 12, a);
    const [x1, y1] = polarToXY(cx, cy, R_TICKS + 4, a);
    majors.push(<line key={`spd-maj-${k}`} x1={x0} y1={y0} x2={x1} y2={y1} stroke="#fff" strokeWidth="2" />);
    const [tx, ty] = polarToXY(cx, cy, R_NUM, a);
    numerals.push(<text key={`spd-num-${k}`} x={tx} y={ty + 4} textAnchor="middle" fontSize="12" fontWeight="700" fill="#e7eaee">{k}</text>);
  }
  for (let k = 10; k < max; k += 10) {
    ...
    minors.push(<line key={`spd-min-${k}`} x1={x0} y1={y0} x2={x1} y2={y1} stroke="#fff" strokeWidth="1.5" />);
  }

  return (
    <svg
      viewBox={`0 0 ${vb} ${vb}`} width={size} height={size}
      role="img" aria-label="Speed Gauge" style={{ overflow: "hidden" }}
    >
...
function MiniGaugeBlack({ label = "GAUGE", value = 50, size = 92, greenToRed = false }) {
  ...
  for (let k = 0; k <= 100; k += 20) {
    ...
    majors.push(<line key={`mini-maj-${k}`} x1={x0} y1={y0} x2={x1} y2={y1} stroke="#fff" strokeWidth="1.8" />);
  }

  return (
    <svg
      viewBox={`0 0 ${vb} ${vb}`} width={size} height={size}
      role="img" aria-label={`${label} gauge`} style={{ overflow: "hidden" }}
    >
      ...
      <g>{majors}</g>
      <path d={arcPath(cx, cy, R_TICKS - 16, START, currentAngle)} stroke="#ef4444" strokeWidth="5" fill="none" strokeLinecap="round" />
      ...
      {lights.map((l) => (
        <div
          ...
          style={{
            ...
            color: "#0b0b0b", background: l.color, boxShadow: "0 0 0 2px rgba(0,0,0,.4) inset",
            opacity: 0.28, filter: "saturate(.7) brightness(.9)",
          }}
        >
