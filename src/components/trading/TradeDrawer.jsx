...
// Paper Trading Drawer (wide/readable) — Core for Options, Streamer for Paper
// - Core (Backend-1):     /api/options/meta  |  /api/options/chain
// - Streamer (Backend-2): /paper/status (SSE) | /paper/execute
//
// Features:
// • Wide panel + larger text + top offset (easy to read, doesn't hide top)
// • Live Positions/Orders from /paper/status (SSE)
// • Options chain dropdown (mark/bid/ask shown), default ATM strike
// • Auto-fill Exec Price for MKT from selected strike's mark (options)
// • Equity MKT uses manual Exec Price (can be automated later)
// • Paper Connected pill
//
// IMPORTANT: Requires hook `usePaperStatus` at src/hooks/usePaperStatus.js
...
/* -------------------------------- helpers -------------------------------- */
const FONT = 14;      // global font size
const FONT_SM = 13;
...
async function safePost(url, body, headers={}) {
  try { const r = await fetch(url, { method:"POST", headers:{ "Content-Type":"application/json", ...headers }, body: JSON.stringify(body) });
    const d = await r.json().catch(()=>null);
    return { ok: r.ok, status:r.status, data:d }; }
  catch(e){ return { ok:false, status:0, data:null }; }
}
...
function Pill({ color = "#374151", children }) {
  return (
    <span style={{
      ...
      background:color, color:"#e5e7eb", fontSize:FONT_SM, fontWeight:700
    }}>
      {children}
    </span>
  );
...
const inputStyle = {
  ...
  border:"1px solid #1f2937", borderRadius:10, padding:"8px 10px", fontSize:FONT
};
const selStyle = { ...inputStyle, appearance:"auto" };
const primaryBtn = {
  background:"#2563eb", color:"#fff", border:"1px solid #1f2937",
  borderRadius:10, padding:"10px 14px", cursor:"pointer", fontWeight:700, fontSize:FONT
};
const ghostBtn = {
  background:"#111827", color:"#e5e7eb", border:"1px solid #1f2937",
  borderRadius:10, padding:"10px 14px", cursor:"pointer", fontSize:FONT
};
const dangerBtn = {
  background:"#b91c1c", color:"#fff", border:"1px solid #1f2937",
  borderRadius:10, padding:"10px 14px", cursor:"pointer", fontWeight:800, fontSize:FONT
};
const chipOn  = { background:"#1f2937", color:"#e5e7eb", border:"1px solid #1f2937", borderRadius:999, padding:"8px 12px", cursor:"pointer", fontSize:FONT, fontWeight:800 };
...
export default function TradeDrawer({ open, onClose, defaultSymbol = "SPY" }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!alive) return;
      if (res.ok && res.data) setStatus({ mode:res.data.mode||"PAPER", liveEnabled:!!res.data.liveEnabled, connected:!!res.data.connected });
    })();
  ...
  const { connected: paperConnected, error: paperError, snapshot: paperSnap } = usePaperStatus();
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (res.ok && Array.isArray(res.data)) {
        ...
        setChainAvail(true);
        // choose a default/ATM-ish strike if none selected yet
  ...
  // Auto-fill Exec Price for MKT options from selected strike's mark
  useEffect(() => {
    ...
    if (!row) return;
    setExecPrice(String(row.mark ?? "")); // if null, leaves as empty
  ...
  useEffect(() => {
    ...
    let alive = true;
    (async () => { const r = await safeGet(URLS.riskStatus);
      if (!alive) return; setRisk({ killSwitch:!!r.data?.killSwitch, caps:r.data?.caps||null, last:new Date().toISOString() }); })();
    return () => { alive=false; };
  ...
  async function handleKillSwitch() {
    ...
    if (res.ok) alert("Kill Switch engaged."); else alert("Kill Switch failed or endpoint not available.");
  }
  ...
  function validateTicket() {
    ...
    if ((type === "STOP" || type === "STOP-LMT") && numberOrNull(stopPrice) == null) errors.push("Stop price required.");
    if (type === "MKT" && (numberOrNull(execPrice) == null || +execPrice <= 0)) errors.push("Exec Price required for MKT.");
  ...
  async function confirmSubmit() {
    ...
    const price = isMkt ? numberOrNull(execPrice) : (numberOrNull(limitPrice) ?? numberOrNull(stopPrice));

    let payload;
    if (assetType === "EQUITY") {
      payload = {
        symbol: symbol.toUpperCase(),
        side: sideIn, qty: Number(equityQty), price: Number(price || 0),
        ts: Math.floor(Date.now()/1000)
      };
    } else {
      payload = {
        symbol: symbol.toUpperCase(), // underlying for MVP
        side: sideIn, qty: Number(contracts), price: Number(price || 0),
        ts: Math.floor(Date.now()/1000),
      };
    }

    const res = await safePost(`${streamBase}/paper/execute`, payload, { "X-Idempotency-Key": cryptoRandom() });
    if (res.ok && res.data?.ok) {
      alert(`Submitted: ${payload.side} ${payload.qty} ${symbol.toUpperCase()} @ ${payload.price}`);
      setTab("orders"); // stream updates the tables
    } else {
  ...
  return (
    <div role="dialog" aria-modal="true" style={{ position:"fixed", inset:0, zIndex:2000, fontSize:FONT }}>
      ...
      {/* Panel (wide, top/bottom offset, rounded) */}
      <div
        style={{
          ...
          boxShadow:"-10px 0 30px rgba(0,0,0,0.55)", overflow:"hidden"
        }}
      >
        {/* Header */}
        <div style={{
        ...
        <div style={{ padding:"8px 12px", borderBottom:"1px solid #1f2937" }}>
          {[
            ["ticket","Ticket"],["positions","Positions"],["orders","Orders"],
            ["executions","Executions"],["journal","Journal"],["options","Options"],["risk","Risk"]
          ].map(([key,label])=>(
            <button key={key} onClick={()=>setTab(key)} style={{
              background: tab===key ? "#1f2937" : "#0f172a",
              color:"#e5e7eb", border:"1px solid #1f2937", borderRadius:999,
              padding:"8px 12px", marginRight:6, cursor:"pointer", fontSize:FONT_SM, fontWeight: tab===key ? 800 : 600
            }}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ flex:1, overflow:"auto", padding:"14px 16px 20px", display:"grid", gap:14 }}>
          ...
          {tab==="ticket" && (
            <div style={{ display:"grid", gap:12 }}>
              ...
              {assetType==="OPTION" && (
                <>
                  <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                    ...
                    <label htmlFor="manualOpt" style={{ color:"#9ca3af" }}>Manual expiration/strike (use if endpoints aren’t live)</label>
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:10 }}>
                    ...
                    {/* Strike powered by chain; fallback to manual */}
                    <div>
                      ...
                      {chainAvail && (chainRows||[]).length ? (
                        <select
                          ...
                          title="Strikes from /api/options/chain"
                        >
                          {(chainRows||[]).map((r)=>(
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:10 }}>
                    ...
                    <div>
                      ...
                      <select style={selStyle} value={type} onChange={(e)=>setType(e.target.value)}>
                        <option>MKT</option><option>LMT</option><option>STOP</option><option>STOP-LMT</option>
                      </select>
                    </div>
                    <div>
              ...
              {assetType==="EQUITY" && (
                <>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(5, 1fr)", gap:10 }}>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>Side</label>
                      <select style={selStyle} value={sideIn} onChange={(e)=>setSideIn(e.target.value)}><option>BUY</option><option>SELL</option></select>
                    </div>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>Shares</label>
                      <input style={inputStyle} type="number" value={equityQty} min={1} onChange={(e)=>setEquityQty(Number(e.target.value))} />
                    </div>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>Type</label>
                      <select style={selStyle} value={type} onChange={(e)=>setType(e.target.value)}><option>MKT</option><option>LMT</option><option>STOP</option><option>STOP-LMT</option></select>
                    </div>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>Limit</label>
                      <input style={inputStyle} type="number" placeholder="—" value={limitPrice} onChange={(e)=>setLimitPrice(e.target.value)} />
                    </div>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>Stop</label>
                      <input style={inputStyle} type="number" placeholder="—" value={stopPrice} onChange={(e)=>setStopPrice(e.target.value)} />
                    </div>
                  </div>

                  <div style={{ display:"grid", gridTemplateColumns:"repeat(2, 1fr)", gap:10 }}>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>Exec Price (paper)</label>
                      <input style={inputStyle} type="number" placeholder="e.g., 500.00" value={execPrice} onChange={(e)=>setExecPrice(e.target.value)} />
                    </div>
                    <div><label style={{ color:"#93c5fd", fontSize:FONT_SM }}>TIF</label>
                      <select style={selStyle} value={tif} onChange={(e)=>setTif(e.target.value)}><option>DAY</option><option>GTC</option></select>
                    </div>
                  </div>
                </>
              )}
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button style={primaryBtn} onClick={submitTicket} disabled={liveBlocked}>Review (paper)</button>
              ...
              {review && (
                <div style={{ border:"1px solid #1f2937", borderRadius:10, padding:12, background:"#111827" }}>
                  ...
                  <div style={{ color:"#9ca3af", fontSize:FONT, marginBottom:8 }}>
                    {assetType==="EQUITY"
                      ? (<>{sideIn} {equityQty} {symbol.toUpperCase()} • {type} {type==="MKT" && `@ ${execPrice}`} {type==="LMT" && `@ ${limitPrice}`} {type==="STOP" && `stop ${stopPrice}`} {type==="STOP-LMT" && `stop ${stopPrice} / limit ${limitPrice}`} • {tif}</>)
                      : (<>{sideIn} {contracts}x {optSide.toUpperCase()} {symbol.toUpperCase()} {strike} {expiration} • {type} {type==="MKT" && `@ ${execPrice}`} {type==="LMT" && `@ ${limitPrice}`} {type==="STOP" && `stop ${stopPrice}`} {type==="STOP-LMT" && `stop ${stopPrice} / limit ${limitPrice}`} • {tif}</>)
                    }
                  </div>
                  <div style={{ display:"flex", gap:10 }}>
                    <button style={primaryBtn} onClick={confirmSubmit}>Confirm</button>
            ...
            <div>
              ...
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead>
                    <tr>{["Symbol","Qty","Avg","Last","Unrealized","Realized","Updated"].map(h=>
          ...
          {tab==="orders" && (
            <div>
              ...
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead>
                    <tr>{["Time","Order ID","Symbol","Side","Qty","Price","Status"].map(h=>
          ...
          {/* Journal / Options / Risk (unchanged content) */}
          {tab==="journal" && (
          ...
          {tab==="options" && (
            <div style={{ color:"#9ca3af", fontSize:FONT }}>
              Options Chain is powered by Core:
              <br/><code>/api/options/meta?symbol=SPY</code>
              <br/><code>/api/options/chain?symbol=SPY&expiration=YYYY-MM-DD&side=call|put</code>
            </div>
          )}
