...
import type { Metrics } from '../types/market';
...
export default function MarketMeters() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try { setData(await fetchMetrics()); } catch (e) { console.error(e); }
  ...
  return (
    <div style={{ display:'grid', gap:12, gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))' }}>
      {data.sectors.map(s => (
        <div key={s.sector} style={{ padding:12, background:'#141414', borderRadius:8 }}>
          <div style={{ fontWeight:700 }}>{s.sector}</div>
          <div style={{ opacity:0.8, fontSize:12 }}>TS: {new Date(data.timestamp * 1000).toLocaleString()}</div>
