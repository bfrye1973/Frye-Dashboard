...
import FerrariDashboard from "./components/FerrariDashboard";
...
export default function App() {
  ...
  // Debug feed banner (kept from earlier so you can see source/backend/mock)
  ...
  const [settings] = useState({
    ...
    // sr: { blockHeightPct: 0.002 },
    // swing: { blockHeightPct: 0.002 },
    // squeeze: { conv: 50, length: 20 },
    // smi: { length: 14, smooth: 3, signal: 3 },
    // vol: { upColor: "#22c55e", downColor: "#ef4444" },
  });

  // Build list passed to the indicator resolver
  ...
  // ----- Candles from the chart (for engine lights & dist-days)
  const [candles, setCandles] = useState([]);

  // ----- Gauges (momentum/breadth) for the current index/symbol
  const [gaugesRow, setGaugesRow] = useState(null);
  useEffect(() => {
    let live = true;
    (async () => {
      const rows = await getGauges(symbol); // backend JSON or CSV (services/gauges)
      if (!live) return;
      // If backend returns multiple rows, pick latest totals row or the first row.
  ...
  useEffect(() => {
    ...
    const next = computeEngineLights({
      candles,
      momentum: m,
      breadth: b,
      prior: lights,
      // psi: real PSI can be wired later if you want fuel to be live
      // srZones: you can pass SR zones here when we expose them
    });
    setLights(next);
  }, [candles, gaugesRow]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0d1117", color: "#d1d4dc" }}>
      ...
      >
        FEED: <strong>{dbg.source}</strong> • bars: <strong>{dbg.bars}</strong> • shape:{" "}
        <strong>{dbg.shape}</strong> • url: <span style={{ opacity: 0.8 }}>{dbg.url}</span>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 16, padding: 16 }}>
        ...
        <div>
          ...
          <div style={panel}>
            ...
            {[
              ...
              ["swing", "Swing / Liquidity"],
              ["squeeze", "Squeeze (LuxAlgo)"],
              ["smi", "SMI"],
              ["vol", "Volume"],
            ].map(([id, lbl]) => (
        ...
        {/* Right side: Ferrari cluster + Gauges + Chart */}
        <div style={{ border: "1px solid #1b2130", borderRadius: 12, overflow: "hidden" }}>
          {/* Ferrari cluster w/ engine lights */}
          <FerrariDashboard
            logoUrl="/ferrari.png" // drop a logo at /public/ferrari.png or use any URL
            momentum={Number(gaugesRow?.momentum ?? 0)}
            breadth={Number(gaugesRow?.breadth ?? 0)}
            psi={undefined} // we can compute PSI later if you want Fuel to be live
            lights={lights}
            onLightClick={(id) => console.log("light:", id)}
          />

          {/* Table gauges (optional — leave if you want both views) */}
          <GaugesPanel defaultIndex={symbol} />

          {/* Chart (multi‑pane version; make sure LiveLWChart accepts onCandles) */}
          <LiveLWChart
            symbol={symbol}
            timeframe={timeframe}
            height={620}
